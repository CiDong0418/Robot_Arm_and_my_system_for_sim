...
#include <mutex> 
std::mutex MotorUnion::hardware_mutex;
	  ...
	  waiting_frequency(10)
{
	...
	{
		// Check if the motor model is valid
		// std::cout << "Motor Model: " << MotorModelArray.at(i) << std::endl;

		if (MotorModelArray.at(i) == "Pro100" || MotorModelArray.at(i) == "Pro200" || MotorModelArray.at(i) == "Pro20" || MotorModelArray.at(i) == "RH")
	...
	if (ConnectAllMotors(MotorUnion::allport)){
		std::cout << "[MotorUnion] All motors connected." << std::endl;
		BGON();
...
const bool MotorUnion::ConnectAllMotors(vector<unsigned char> &AllPortNumber)
{
	...
	for (it = AllPortNumber.begin(); it != AllPortNumber.end();)
	{
		...
		// std::cout<<"Checking port /dev/ttyUSB"<<(int)(*it)<<std::endl;
		...
		tmp_portHandler = dynamixel::PortHandler::getPortHandler(port_path.c_str());
		...
		for (int i = 0; i < Motor_Union.size(); i++)
		{
			if (Motor_Union.at(i)->GetMotorConnected() == false)
			{
				...
				// std::cout<<"tmp_portHandler: "<<tmp_portHandler<<std::endl;
				// std::cout<<"packetHandler: "<<packetHandler<<std::endl;
				// std::cout<<"tmp_groupBulkRead: "<<tmp_groupBulkRead<<std::endl;
				// std::cout<<"tmp_groupBulkWrite: "<<tmp_groupBulkWrite<<std::endl;
	...
	bool connected = true;
	// std::cout<< Motor_Union.size()<<std::endl;
	for (int i = 0; i < Motor_Union.size(); i++){
		connected &= Motor_Union.at(i)->GetMotorConnected();

		std::cout<<"Motor "<<i<<" connected: "<<Motor_Union.at(i)->GetMotorConnected()<<std::endl;
	}
...
{
	// std::cout<<"Reading motor "<<&idx<<" angle."<<std::endl;
	// for (int i = 0; i < Motor_Union.size(); i++){
		// Motor_Union.at(i)->ReadData();
		// std::cout<<"Motor "<<i<<" angle: "<<Motor_Union.at(i)->GetMotor_PresentAngle()<<std::endl;
	// }	
...
{
	// Add parameters
	std::lock_guard<std::mutex> lock(hardware_mutex); 
	bool is_Write = false;
	for (int i = 0; i < Motor_Union.size(); i++)
...
void MotorUnion::ReadData() const
{
	...
	for (int i = 0; i < Motor_Union.size(); i++){
		Motor_Union.at(i)->ReadData();
		// std::cout << "[Dmotor] ID=" << int(i)
            //   << " new angle=" << Motor_Present_Angle << std::endl;
		// std::cout<<"Motor "<<i<<" angle: "<<Motor_Union.at(i)->GetMotor_PresentAngle()<<std::endl;
	}
