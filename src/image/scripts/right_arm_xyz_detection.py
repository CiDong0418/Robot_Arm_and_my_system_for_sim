#!/usr/bin/env python3
"""
right_arm_xyz_detection.py
==========================
ROS 節點：右手相機物體 XYZ + 半徑 R 偵測服務。

職責：
    - 訂閱 ROS Topic 獲取即時畫面與對齊的深度圖（不再獨佔 USB 硬體）
    - 自動獲取相機內參 (CameraInfo)
    - 持續顯示即時畫面（背景執行緒）
    - 訂閱 /right_arm_detection/target 觸發 GroundingDINO 推論
    - 發布 3D 結果到 /right_arm_detection/xyz 等頻道（單位：mm）
"""
import os
import sys
import time
...
def _draw_bbox(frame: np.ndarray, x1: int, y1: int, x2: int, y2: int,
    ...
    green, orange, font, font_scale = (0, 255, 0), (0, 200, 255), cv2.FONT_HERSHEY_SIMPLEX, 0.55
    ...
    cv2.rectangle(frame, (x1, y1 - text_h - 8), (x1 + text_w + 4, y1), green, -1)
    cv2.putText(frame, label, (x1 + 2, y1 - 4), font, font_scale, (0, 0, 0), 1, cv2.LINE_AA)

    radius_label = f"R = {radius_mm:.1f} mm"
    (radius_w, radius_h), _ = cv2.getTextSize(radius_label, font, font_scale, 1)
    cv2.rectangle(frame, (x1, y2), (x1 + radius_w + 4, y2 + radius_h + 8), orange, -1)
    cv2.putText(frame, radius_label, (x1 + 2, y2 + radius_h + 2), font, font_scale, (0, 0, 0), 1, cv2.LINE_AA)


class RightArmXYZDetectionNode:
    def __init__(self):
        rospy.init_node("right_arm_xyz_detection", anonymous=False)
        ...
        self._intrinsics_dict = None
        self._depth_scale = 0.001
        self._camera_frame_id = "right_arm_camera_color_optical_frame"
        ...
        if not os.path.isfile(dino_config_path):
            raise FileNotFoundError(f"[right_arm_xyz] GroundingDINO config 不存在: {dino_config_path}")
        if not os.path.isfile(dino_weights_path):
            raise FileNotFoundError(f"[right_arm_xyz] GroundingDINO weights 不存在: {dino_weights_path}")
        if GroundingDINODetector is None:
            raise RuntimeError(
                f"[right_arm_xyz] 無法載入 GroundingDINO 依賴：{_GROUNDING_IMPORT_ERROR}。"
                "請先安裝 torch 與 groundingdino。"
            )

        rospy.loginfo("[right_arm_xyz] 載入 GroundingDINO 模型...")
        ...
        self._pub_xyz = rospy.Publisher("/right_arm_detection/xyz", PointStamped, queue_size=1)
        self._pub_radius = rospy.Publisher("/right_arm_detection/radius", Float32, queue_size=1)
        self._pub_status = rospy.Publisher("/right_arm_detection/status", String, queue_size=1)

        rospy.Subscriber("/right_arm_detection/target", String, self._on_target_received, queue_size=1)
        rospy.Subscriber("/right_arm_camera/color/camera_info", CameraInfo, self._camera_info_callback)

        color_sub = message_filters.Subscriber('/right_arm_camera/color/image_raw', Image)
        depth_sub = message_filters.Subscriber('/right_arm_camera/aligned_depth_to_color/image_raw', Image)
        ...
        rospy.loginfo("[right_arm_xyz] 系統就緒，等待相機畫面與辨識指令。")
    ...
    def _sync_image_callback(self, color_msg, depth_msg):
        try:
            ...
            rospy.logerr(f"CV Bridge Error: {error}")

    def _run_detection(self, target: str, color_image, depth_image):
        if self._intrinsics_dict is None:
            rospy.logwarn("[right_arm_xyz] 尚未收到 CameraInfo，無法計算。")
            return
        ...
        radius = compute_object_radius_3d(self._intrinsics_dict, x1, y1, x2, y2, depth_median)
        ...
        self._publish_radius(radius_mm)
        self._publish_status(
            f"OK | target={target} XYZ=({xyz_mm[0]:.3f}, {xyz_mm[1]:.3f}, {xyz_mm[2]:.3f})mm R={radius_mm:.1f}mm"
        )

    def _display_loop(self) -> None:
        win = "Right Arm Detection | press q to close"
    ...
    def _on_target_received(self, msg: String) -> None:
        with self._target_lock:
            self._current_target = msg.data.strip()
    ...
    def _publish_radius(self, radius_value: float) -> None:
        self._pub_radius.publish(Float32(data=float(radius_value)))
