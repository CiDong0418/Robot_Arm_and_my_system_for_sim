...
static const Eigen::Matrix<double,7,4> left_arm_DH_table = (Eigen::Matrix<double,7,4>() <<
    0,     0,      0,    233,
    0, -M_PI/2,    0,      0,
    0,     0, 265.7,      0,
    0, -M_PI/2,   30,      0,
...
LeftArm::LeftArm()
    // : ArmInfo(left_arm_DH_, 0, 6, {0, 1, 2, 3, 4, 5, 31}, {"Pro200", "Pro200", "Pro100", "Pro100", "Pro20", "Pro20", "RH"}),
    : ArmInfo(left_arm_DH_table,  {0, 1, 2, 3, 4, 5, 32}, {"Pro200", "Pro200", "Pro100", "Pro100", "Pro20+", "Pro20+", "RH"}),
        ...
        last_position_(Vector3d::Zero())
{
    ...
    std::cout << "Class constructed: Left Arm" << std::endl;
    std::cout << "================================================================================" << std::endl;
}
...
Quaterniond LeftArm::quatConjugate(const Quaterniond& q) {
    return Quaterniond(q.w(), -q.x(), -q.y(), -q.z());
}

Quaterniond LeftArm::quatMul(const Quaterniond& a, const Quaterniond& b) {
    return a * b;
}


bool LeftArm::stepIK(double ox_deg, double oy_deg, double oz_deg,
                   double px, double py, double pz,  double acceleration_factor)
{
    ...
    // Error quaternion
    // if(q_current.dot(q_target)<0) q_target.coeffs() *=-1;// Ensure shortest path
...
bool LeftArm::step_move(double ox_deg, double oy_deg, double oz_deg, double px, double py, double pz, double acceleration_factor) 
{
    updateRobotPose();
...
// bool LeftArm::stepMoveJ(const Eigen::Matrix<double,6,1>& target_joints, double max_speed_rad, double acceleration_factor) {
...
//     if (max_diff < 0.03) { 
//         setJointVelocity(Eigen::Matrix<double,6,1>::Zero()); // 停機
//         return true; // 到達目標
//     }
...
void LeftArm::testing() {
    double before = GetMotor_PresentAngle(FIRST_HAND_ID_ + 0) * Angle2Rad;
    SetMotor_Velocity(FIRST_HAND_ID_ + 0, 0.1 * pro_radpersec2scale_);
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    std::cout << "Set velocity command to motors." << std::endl;
    double m1 = GetMotor_PresentAngle(FIRST_HAND_ID_ + 0) * Angle2Rad;
    ...
    Stop_all_motors();
    double m2 = GetMotor_PresentAngle(FIRST_HAND_ID_ + 0) * Angle2Rad;
    std::cout << "m2: " << m2 << std::endl;

    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    double after = GetMotor_PresentAngle(FIRST_HAND_ID_ + 0) * Angle2Rad;
    std::cout << "before: " << before << ", after: " << after << std::endl;

}

void LeftArm::Angle_adjust(double ox_deg, double oy_deg, double oz_deg, bool mode_change)
{
    updateRobotPose();
...
void LeftArm::trajectory_planning(double ox, double oy, double oz, double px, double py, double pz, bool mode_change)
{
...
void LeftArm::Trajectory_Planning_NoCheck(double ox_deg, double oy_deg, double oz_deg, double X, double Y, double Z)
{
    if(is_working_) std::cout << "\t[INFO] Arm is moving. Wait for the arm to stop!" << std::endl;
        ...
        std::cout << "\t[INFO] Start trajectory planning to (" << X << ", " << Y << ", " << Z << ")"
                  << " with orientation (" << ori_deg[0] << ", " << ori_deg[1] << ", " << ori_deg[2] << ")" << std::endl;
...
void LeftArm::mode_controll(double X, double Y, double Z, std::string pick_mode, double angle)
{
    ...
    auto p = getPickTypePose("left", pick_mode, angle);
...
void LeftArm::setpositionPose(double x, double y, double z) {
    updateRobotPose();
    double init_ox = left_orientation_deg_[0];
    double init_oy = left_orientation_deg_[1];
    double init_oz = left_orientation_deg_[2];
...
void LeftArm::updateRobotPose() {
    ...
    left_world_pos_ = pos4.head<3>();
    left_orientation_deg_ = getCurrentOrientationInDegree();
    getArmInformation();
    
}
void LeftArm::getArmInformation() const {
    cout<<"==================================="<<endl;
    cout<<"Current left base Position = "<<getCurrentPosition().transpose()<<endl;
    ...
    cout<<"Current left Joint Angles(deg) = "<<(getMotorAngle()*180.0/M_PI).transpose()<<endl;
    cout<<"==================================="<<endl;
}
...
bool LeftArm::GetWorkingState() { return is_working_; }



int LeftArm::Sign(float x) { return (x >= 0 ? 1 : -1); }
